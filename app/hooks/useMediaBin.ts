...
export const useMediaBin = (
  ...
  */
  ...
  // Add media item directly to media bin (for AI-generated or downloaded media)
  const handleAddMediaItemToBin = useCallback((mediaItem: Partial<MediaBinItem>) => {
    const newItem: MediaBinItem = {
      id: mediaItem.id || generateUUID(),
      name: mediaItem.name || "Untitled Media",
      mediaType: mediaItem.mediaType || "image",
      mediaUrlLocal: mediaItem.mediaUrlLocal || null,
      mediaUrlRemote: mediaItem.mediaUrlRemote || null,
      durationInSeconds: mediaItem.durationInSeconds || 0,
      media_width: mediaItem.media_width || 0,
      media_height: mediaItem.media_height || 0,
      text: mediaItem.text || null,
      isUploading: false,
      uploadProgress: null,
      left_transition_id: null,
      right_transition_id: null,
    ...
    setMediaBinItems(prev => [...prev, newItem]);
    console.log("Added media item to bin:", newItem.name);
  }, []);
