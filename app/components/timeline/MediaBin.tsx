...
export default function MediaBin() {
  ...
  // Handle downloading media from external sources
  const handleDownloadMedia = useCallback(
    async (url: string, filename: string, type: "image" | "video" | "audio") => {
      if (!projectId) {
      ...
      try {
        // Call the download API - it saves the file to the 'out' folder
        ...
        // Add directly to media bin without re-uploading
        // The file is already saved to the 'out' folder by the download API
        onAddMediaItem({
          name: filename,
          mediaType: type,
          mediaUrlLocal: null,
          mediaUrlRemote: data.fullUrl, // Full URL to render server: http://localhost:8000/media/{filename}
          durationInSeconds: 0, // Will be calculated when added to timeline
        });
        
        console.log("Media downloaded and added to bin:", filename);
      } catch (error) {
        console.error("Error downloading media:", error);
      }
    },
    [projectId, onAddMediaItem]
  );
