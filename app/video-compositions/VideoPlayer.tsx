...
import type { VisualEffect, ColorCorrection, BlendMode } from "../components/timeline/advanced-types";
...
export function TimelineComposition({
  ...
  const createMediaContent = (scrubber: TimelineDataItem['scrubbers'][0] | ScrubberState): React.ReactNode => {
    ...
    // Wrap content with EffectLayer if effects exist
    // Get effects from scrubber (will be populated from timeline state)
    const visualEffects = (scrubber as any).visualEffects as VisualEffect[] | undefined;
    const colorCorrection = (scrubber as any).colorCorrection as ColorCorrection | undefined;
    const blendMode = (scrubber as any).blendMode as BlendMode | undefined;
    const opacity = (scrubber as any).opacity as number | undefined;

    // Only wrap with EffectLayer if there are effects to apply
    if (visualEffects || colorCorrection || blendMode || (opacity !== undefined && opacity !== 1)) {
      content = (
        <EffectLayer
