...
import { AdvancedPanels } from "~/components/editor/AdvancedPanels";
...
export default function TimelineEditor() {
  ...
  const [showAdvancedPanels, setShowAdvancedPanels] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchStarCount = async () => {
      try {
        const response = await fetch("https://api.github.com/repos/SoloWave-Lab/Kylo");
        if (response.ok) {
    ...
    <div
      ...
      <header className="h-9 border-b border-border/50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex items-center justify-between px-3 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-medium tracking-tight">Kylo Studio</h1>
        </div>
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-1">
        ...
        <ResizablePanel defaultSize={20} minSize={15} maxSize={40}>
          <div className="h-full border-r border-border">
            <LeftPanel
              ...
              onAddMediaItem={handleAddMediaItemToBin}
              ...
              <div className="h-full flex flex-col bg-muted/20">
                <div className="h-8 border-b border-border/50 bg-muted/30 flex items-center justify-between px-3 shrink-0">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Separator orientation="vertical" className="h-4 mx-1" />
                    <Button 
                      variant={showAdvancedPanels ? "default" : "ghost"}
                      size="sm" 
                      onClick={() => setShowAdvancedPanels(!showAdvancedPanels)} 
                      className="h-6 px-2 text-xs"
                      title="Toggle Advanced Tools (Animation & Audio)">
        ...
        {/* Advanced Panels - Animation & Audio (toggleable) */}
        {showAdvancedPanels && (
          <>
            <ResizableHandle withHandle />
            <ResizablePanel defaultSize={25} minSize={20} maxSize={40}>
              <div className="h-full border-l border-border">
                <AdvancedPanels 
                  projectId={projectId} 
                  selectedClipId={selectedItem || undefined}
                  onClose={() => setShowAdvancedPanels(false)}
                />
              </div>
            </ResizablePanel>
          </>
            ...
            <ResizablePanel defaultSize={20} minSize={15} maxSize={35}>
              <div className="h-full border-l border-border">
                <ChatBox
                  ...
                  onAddMediaToLibrary={handleAddMediaItemToBin}
                />
              </div>
            </ResizablePanel>
          </>
