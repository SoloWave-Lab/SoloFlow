...
export async function loader({ request }: { request: Request }) {
  const { getProjectById, listProjectsByUser } = await import("~/lib/projects.repo.server");
  const { listAssetsByUser, softDeleteAsset } = await import("~/lib/assets.repo.server");
  const { loadProjectState } = await import("~/lib/timeline.store.server");
  ...
  if (m && request.method === "DELETE") {
    ...
    const { deleteProjectById } = await import("~/lib/projects.repo.server");
...
export async function action({ request }: { request: Request }) {
  const { createProject, getProjectById, deleteProjectById } = await import("~/lib/projects.repo.server");
  const { listAssetsByUser, softDeleteAsset } = await import("~/lib/assets.repo.server");
  const { loadProjectState, saveProjectState } = await import("~/lib/timeline.store.server");
  const fs = await import("fs");
  const path = await import("path");
