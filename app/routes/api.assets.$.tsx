...
async function inferMediaTypeFromName(
  name: string,
...
export async function loader({ request }: { request: Request }) {
  const { listAssetsByUser, getAssetById } = await import("~/lib/assets.repo.server");
...
export async function action({ request }: { request: Request }) {
  const { insertAsset, getAssetById, softDeleteAsset } = await import("~/lib/assets.repo.server");
  const fs = await import("fs");
  const path = await import("path");
  const OUT_DIR = path.resolve("out");
